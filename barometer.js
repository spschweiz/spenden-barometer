class Barometer {
  ...
  async render() {
    await fetch(
      `https://tel.sp-ps.ch/raisenow/barometer/${this.campaign}/${this.campaign_is_id}`
    )
      .then((response) => response.json())
      .then((data) => (this.data = data));
    let stats = this.data.result.facets[0].stats;
    let amount_left =
      this.goal - (this.offline_amount + parseInt(stats.total) / 100);
    let percentage = (1 - amount_left / this.goal) * 100;
    if (!this.baro_show_revenue && this.goal_backers != 0) {
      percentage =
        ((this.offline_backers + stats.count) / this.goal_backers) * 100;
    }
    ...
    if (this.show_donation_revenue) {
      ...
      let total = Math.floor(
        stats.total / 100 + this.offline_amount
    ...
    if (
      ...
      let amount_financed = Math.floor(
        (this.offline_amount + parseInt(stats.total) / 100) /
    ...
    if (this.show_text_amount_backers) {
      ...
      let count_backers = (stats.count + this.offline_backers).toLocaleString(
        "de-CH"
      );
    ...
    if (this.mean) {
      let mean = Math.floor(
        (stats.total / 100 + this.offline_amount) /
          (this.offline_backers + stats.count)
      ).toLocaleString("de-CH");
