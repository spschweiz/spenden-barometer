class Barometer {
    constructor(
      ...
      this.show_text_amount_backers_with_goal =
        show_text_amount_backers_with_goal;
    ...
    async render() {
      await fetch(
        `https://tel.sp-ps.ch/raisenow/main/${this.campaign}/${this.campaign_is_id}`
      )
        .then((response) => response.json())
        .then((data) => (this.data = data));
      let amount_left =
        this.goal - (this.offline_amount + parseInt(this.data.total) / 100);
      let percentage = (1 - amount_left / this.goal) * 100;
      if (!this.baro_show_revenue && this.goal_backers != 0) {
        percentage =
          ((this.offline_backers + this.data.count) / this.goal_backers) * 100;
      }
      ...
      baro_progress.style.backgroundColor = this.progress_color;
      baro_border.style.backgroundColor = this.back_color;
      ...
      if (this.show_donation_revenue) {
        ...
        let total = Math.floor(
          this.data.total / 100 + this.offline_amount
        ).toLocaleString("de-CH");
  
        if (this.lang == "de") {
          if (this.show_donation_revenue_with_goal) {
            baro_text_goal.innerHTML = `<b>${total}</b> von ${this.goal.toLocaleString(
              "de-CH"
            )} Franken gesammelt`;
          } else {
          ...
          if (this.show_donation_revenue_with_goal) {
            baro_text_goal.innerHTML = `<b>${total}</b> francs sur ${this.goal.toLocaleString(
              "de-CH"
            )} récoltés`;
          } else {
          ...
          if (this.show_donation_revenue_with_goal) {
            baro_text_goal.innerHTML = `Raccolti <b>${total}</b> di ${this.goal.toLocaleString(
              "de-CH"
            )} franchi`;
          } else {
      ...
      if (
        this.donation_object.length > 0 &&
        this.donation_object_price != 0 &&
        this.show_text_donation_object
      ) {
        let donation_object_quantity = Math.floor(
          this.goal / this.donation_object_price
        ).toLocaleString("de-CH");
        ...
        let amount_financed = Math.floor(
          (this.offline_amount + parseInt(this.data.total) / 100) /
            this.donation_object_price
        ).toLocaleString("de-CH");
      ...
      if (this.show_text_amount_backers) {
        ...
        let count_backers = (this.data.count + this.offline_backers).toLocaleString(
          "de-CH"
        );
      ...
      if (this.mean) {
        let mean = Math.floor(
          (this.data.total / 100 + this.offline_amount) /
            (this.offline_backers + this.data.count)
        ).toLocaleString("de-CH");
